...
- In everyday use:
    ...
    - Once all checks pass, merge the PR to build the project from the main branch and publish a pre-release branch to Github package manager
    - To release a full version, create a new release that is tagged with the current version in main
        - This will build main, publish a full release to Github package manager, and also publish the full version to NuGet
...
- Copy the `example_caller_workflow.txt` and place it in the `.github/workflows` folder of the repository that needs to run the actions. As an example, this file will be placed in the `.github/workflows` folder for [bonsai-onix1](https://github.com/open-ephys/bonsai-onix1).
    ...
    - Note that the actions will not run unless the file is renamed. Specifically, it should be renamed so that the extension is **.yml** (e.g., `build.yml`). The name in front of the extension is not critical, but it will show up in the Actions tab of the repository, so using a meaningful name is always better.
...
- Create rulesets to ensure that the actions are being used correctly as a check during PRs and commits. To do so, navigate to the calling repository, then go to Settings -> Rules -> Rulesets -> New ruleset -> New branch ruleset
    ...
    - Check the following options:
        ...
        - Require status checks to pass by hitting Add checks and search for the following actions (**Note**: these checks might not appear if no actions have been run. In that case, ensure that the actions are in the correct folder in the main branch, then open a PR or push a new commit to an existing PR to trigger actions to run, then come back to this step)
            - check-semver
